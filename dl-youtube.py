...
class DLYoutube(object):
    ...
    CACHE_FOLDER      = os.path.join(TEMP_FOLDER, 'cache') # must contains 'cache' or 'tmp'
    ...
    def __init__(self, **kwargs):
        ...
        try:
            ...
            if not os.path.isdir(self.CACHE_FOLDER):
                raise DLFolderError('Create cache folder failed {}'.format(self.CACHE_FOLDER))
        # setup logger
        ...
        self.logger.info('INFO:: Cache folder          : ' + self.CACHE_FOLDER)
    ...
    def main(self):
        ...
        if self.rm_cache_dir:
            if os.path.isdir(self.CACHE_FOLDER):
                # self.CACHE_FOLDER will be recreated by youtub-dl later during download
                self.logger.info('INFO:: Removing cache folder: {}'.format(self.CACHE_FOLDER))
                shutil.rmtree(self.CACHE_FOLDER, ignore_errors=True)
            else:
                self.logger.debug('DEBUG:: Not removing cache folder: {}. It does not exist!!'.format(self.CACHE_FOLDER))
